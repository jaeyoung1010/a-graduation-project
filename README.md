...
## 담당하였던 역할: Firebase 로그인

조원 **최재영**은 Firebase Authentication을 활용한 이메일/비밀번호 기반 회원가입 및 로그인 기능을 구현했습니다. 입력값을 `TextEditingController`로 관리하고, 회원가입 시 `createUserWithEmailAndPassword`, 로그인 시 `signInWithEmailAndPassword`를 호출하도록 구성했습니다. 인증에 성공한 경우에만 홈으로 이동시키고, 성공·실패 결과는 SnackBar로 즉시 안내하도록 했습니다.

이 기능을 구현하면서 로그인 화면은 입력 UI만으로 완성되지 않으며, 비동기 인증 결과에 따라 화면 전환과 오류 피드백을 일관되게 처리해야 한다는 점을 확인했습니다. 특히 `UserCredential`을 통해 인증 성공 여부를 확인한 뒤 다음 화면으로 이동시키는 흐름을 구성하며, 사용자 경험 측면에서는 모호한 실패보다 이해하기 쉬운 안내가 중요하다는 점을 배웠습니다. 이후 Firestore 비상연락처 기능까지 연결하면서, 실제 서비스에서는 인증된 사용자 ID를 데이터에 연결하고 보안 규칙으로 접근 범위를 제한해야 한다는 필요성도 인식했습니다.
